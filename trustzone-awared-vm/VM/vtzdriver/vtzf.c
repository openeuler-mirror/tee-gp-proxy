		...
		if (shared_mem) {
			if (shared_mem->user_virt_addr ==
				...
				if (shared_mem->kernel_virt_addr) {
					dealloc_res_shm(shared_mem->kernel_virt_addr);
...
static long vtzf_ser_test(struct file *file, unsigned int cmd, void *argp)
{
	...
	uint32_t seq_num = get_seq_num(0);
	int ioctl_args;
	struct_packet_cmd_test packet_cmd = {0};
	...
	struct_packet_cmd_send_cmd *packet_cmd, int index, uintptr_t addrs[][3])
{
	...
	user_buffer = (void *)(clicontext->params[index].memref.buffer
				| (uint64_t)clicontext->params[index].memref.buffer_h_addr << ADDR_TRANS_NUM);
	tloge("____________ user_buffer = %p \n", user_buffer);
	...
	tlogi("alloc for share mem \n");
	tlogi("block_buf = %lx \n", block_buf);
	tlogi("block_buf_size = %u \n", block_buf_size);
	...
	tlogd("clicontext->params[index].memref.h_offset = %u, 0x%lx\n",
	...
	struct tc_ns_client_context *context)
{
	...
	for (i = 0;i < TEE_PARAM_NUM; i++) {
		//tlogi("packet_cmd.block_size[%d] = %u\n",i,packet_cmd.block_size[i]);
		total_buf_size  += packet_cmd.block_size[i];
	}
	packet_cmd.packet_size = total_buf_size;
	//tlogi("total_buf_size = %u \n", total_buf_size);
	...
	for (i = 0; i < TEE_PARAM_NUM; i++) {
		...
		//dump_buff((char*)cmd_buf, total_buf_size);
	...
	free_for_params(&packet_rsp.cliContext, context, addrs);
	kfree(cmd_buf);
	for (i = 0; i < TEE_PARAM_NUM; i++) {
	...
	const void __user *buffer)
{
	...
	struct_packet_cmd_login packet_cmd = {0};
	...
	if (!buffer) {
		packet_cmd_non.packet_size = sizeof(packet_cmd);
...
static int get_agent_buf(struct vtzf_dev_file *dev_file, struct vtz_shared_mem *shared_mem,
	struct_packet_cmd_regagent *packet_cmd, void **bufferp, void **user_addrp)
{
	size_t size = 0;
	void *buffer = NULL;
	void *user_addr = NULL;
...
static int ioctl_register_agent(struct vtzf_dev_file *dev_file, void __user *argp)
{
	...
	if (get_agent_buf(dev_file, shared_mem, &packet_cmd, &buffer, &user_addr)) {
		return -1;
	}
