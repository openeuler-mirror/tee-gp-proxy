...
ssize_t serial_read(struct file *fp_serialport, void *wr_buf, uint32_t size_wr_buf, loff_t *off)
{
	mm_segment_t old_fs;
	ssize_t result;
	result = fp_serialport->f_op->read(fp_serialport, wr_buf, size_wr_buf, off);
	old_fs = get_fs();
	set_fs(get_ds());
	return result;
}
...
int rd_thread_func(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (g_destroy_rd_thread)
			break;
		off = 0;
		//ssize_ret = kernel_read(file->filep, file->buffer + file->offset, SERIAL_PORT_BUF_LEN - file->offset, &off);
		ssize_ret = serial_read(file->filep, file->buffer + file->offset, SERIAL_PORT_BUF_LEN - file->offset, &off);
...
ssize_t serial_write(struct file *fp_serialport, void *wr_buf, uint32_t size_wr_buf, loff_t *off)
{
	mm_segment_t old_fs;
	ssize_t result;
	result = fp_serialport->f_op->write(fp_serialport, wr_buf, size_wr_buf, off);
	old_fs = get_fs();
	set_fs(get_ds());
	return result;
}
...
int wr_thread_func(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if(!write_data)
			continue;
		//ret = kernel_write(fp_serialport, write_data->wr_buf, write_data->size_wr_buf, &off);

		ret = serial_write(fp_serialport, write_data->wr_buf, write_data->size_wr_buf, &off);
...
int serial_port_init(void)
{
	...
	for (i = 0; i < SERIAL_PORT_NUM; i++) {
		...
		if (!file->f_op->read_iter) {
			tlogw("waring! file->f_op->read_iter undefine!\n");
		}
