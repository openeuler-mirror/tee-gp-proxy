...
static int process_address(struct_packet_cmd_send_cmd *packet_cmd, ClientParam params[])
{
    ...
    for (index = 0; index < TEEC_PARAM_NUM; index++) {
        ...
        if (IS_PARTIAL_MEM(paramTypes[index])) {
		    ...
		    //void *test_buff = (void *)(uint64_t)(block_buf[0].block.user_addr +
			//    packet_cmd->cliContext.params[index].memref.h_offset + packet_cmd->cliContext.params[index].memref.offset);
            //printf(" test_buf = %p, block_buf[0].block.user_addr = %p\n", test_buff, (void *)block_buf[0].block.user_addr);
        }
    }// end for
    if (icount ==0) {
